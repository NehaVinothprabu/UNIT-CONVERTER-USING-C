...
int main(){
    ...
    const char *fUnit,*tUnit; // Changed from char* to const char* to avoid warnings
    ...
    scanf(" %c%c%Lf",&fromUnit,&toUnit,&fromValue); // Added a space before %c to consume any leading whitespace
    ...
    if(power==0)
         ...
         printf("%.4Lf %s = %.4Lf*10^%d %s",fromValue,fUnit,toValue,power,tUnit); // Corrected the variable name to toValue
    }
